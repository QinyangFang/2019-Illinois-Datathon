...
par(mfrow = c(1,1))
acf.stock = acf(stock[c(1:breakpoint),], main='ACF Plot', lag.max=40)
pacf.stock = pacf(stock[c(1:breakpoint),], main='PACF Plot', lag.max=40)

auto.arima(log(stock_prices),trace=T)

# Initialzing a dataframe for the forecasted return series
forecasted_series = data.frame(Forecasted = numeric())
arima1 <- arima(stock_prices, order=c(1,1,1), seasonal=list(order=c(1,1,1)),method="ML")
airforecast <- forecast(arima1,h=60,level=c(80,95))
plot(airforecast,ylim=c(0,200))
airforecast